...
| 8 | [`cerebras_memoryx.html`](cerebras_memoryx.html) | **Cerebras MemoryX — cheap DDR, decoupled from the wafer** — the one *shipping* architecture on STRATA-V's thesis (commodity DRAM capacity split from expensive compute), read as an existence-proof. MemoryX is a DRAM+flash hybrid (CS-2) → **commodity DDR5 in 1U servers** (CS-3, Sandia 55 TB), streaming one weight copy to N wafers over Ethernet at **~150 GB/s each** — the *opposite* mechanism to ours (it **deletes** random access; we **emulate** it). The tell: for latency-critical **inference**, Cerebras abandons external DRAM entirely for on-wafer SRAM (WSE-3 = **44 GB @ 21 PB/s**) — which *vindicates* the niche STRATA-V occupies. Includes the SRAM-vs-DDR5 hierarchy contrast (~540,000× bandwidth-per-byte). Evidence in `pricing_evidence/cerebras/`. |
...
- **The win is capacity, not bandwidth.** 1.79 TB/s is reachable from ~18 MRDIMM (or ~35–45 DDR5) channels — but that costs *far* more die-edge, pins, and power than GDDR7 (see the bandwidth-density catch below). DDR5 is *not* a cheap route to bandwidth; it is the only cheap route to multi-TiB *capacity* at that bandwidth.
...
- **The thesis has a hyperscale existence-proof — pointed the other way.** Cerebras MemoryX ships the STRATA-V economics (commodity DDR5, capacity decoupled from compute, one weight copy shared across many engines) at 82 TB / 64-system scale — but by *deleting* random access and streaming weights over Ethernet at ~150 GB/s/wafer, then retreating to on-wafer SRAM (WSE-3 = 44 GB @ 21 PB/s) the moment latency matters. That both validates the economics and confirms the native-interface-impersonation niche is empty (doc 8).
- **The converter silicon, costed.** With NRE discarded, a full 1.79 TB/s converter (1 root + 9 leaf dies, TSMC N5) is **~$906/system at volume** (10k units; ~$813 floor at 50k+) — substrate + at-speed-test/screening dominated, not the N5 wafers — i.e. **~5% of the K3 DRAM bill**; small runs are tooling-dominated (thousands per unit). (Full model in strata_v §13.)
- **Why nobody builds GPUs on DDR5 — and the design-in paradox (the honest catch).** The undeniable reason is **bandwidth density**: delivering 1.79 TB/s from DDR5-class PHYs needs ~1.5–3.2× the *entire perimeter* of the largest GPU die ever made, and ~3.5–6× a native card's memory-subsystem power — DDR5 loses on bits/s per mm of die-edge, per ball, and per watt *by construction*. STRATA-V doesn't repeal this; it **relocates and pays** it (that is what the converter + appliance + power tax *are*), so it wins **only as a capacity play** (>128 GB per bus, where GDDR7 can't reach and HBM is ruinous). Deeper still — the **design-in paradox**: impersonation's only payoff is "GPU die unchanged," but it *requires* vendor design-in, and with design-in a plain UCIe/CXL side-port beats it on shoreline, energy, and flow-control determinism — which is why *everyone* who has design-in (Grace, MI300, SambaNova SN40L, Credo/Celestial/Eliyan) chose the side-port. The thesis survives only in the thin niche *"derivative controller respin, PHY frozen, capacity-bound appliance inference."* (Full accounting: [`strata_v_design.html`](strata_v_design.html) §06.)
